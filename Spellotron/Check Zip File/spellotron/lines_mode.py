...
def lines_algorithm(text, mispelled_words, unknown_words, word_tally):
    """
    ...
    True Spell Correction Process:
        ...
        Step 1.4: Remove case from word then spell check again. If word is legal, skip
                  to step 2.2.
        Step 1.5: If word cannot be found legal, append original word to unknown and
                  corrected.
        Step 2.1: If word is found legal normally or by removing punctuation, append the
                  word to corrected. If it had punctuation it is replaced.
        Step 2.2: If the word is found to be illegal. The word will be returned corrected
                  with the case and punctuation being replaced if needed. The corrected
                  word is appended to corrected words.
...
def lines_file_read(file):
    """
    Takes in a file and spell checks it. Reprints the entire text file corrected
    ...
    for line in document:
        ...
        if line == "":
            pass
        results = lines_algorithm(line, mispelled_words, unknown_words, word_tally)
        mispelled_words = results[0]
        unknown_words = results[1]
        word_tally = results[2]
        corrected_words = " ".join(results[3])
...
def lines_mode():
    """
    Takes in an input and spell checks it. Reprints the entire input corrected
    as well as a list of unknown and mispelled words along with the total
    amount of words in the file. Also reports how long it took to run the
    process.
    :param file: File being spell corrected.
    ...
    print("")
    text = text.split()
    results = lines_algorithm(text, mispelled_words, unknown_words, word_tally)
    mispelled_words = results[0]
    unknown_words = results[1]
    word_tally = results[2]
    corrected_words = " ".join(results[3])
